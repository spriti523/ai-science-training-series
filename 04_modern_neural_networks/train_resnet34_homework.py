...
def train_epoch(i_epoch, step_in_epoch, train_ds, val_ds, network, optimizer, BATCH_SIZE, checkpoint):
    ...
    steps_per_epoch = int(15000/ BATCH_SIZE) #1281167
    steps_validation = int(1500 / BATCH_SIZE)
    acc_new=np.zeros(steps_per_epoch)
    loss_new=np.zeros(steps_per_epoch)
    start = time.time()
    for train_images, train_labels in train_ds.take(steps_per_epoch):
        ...
        acc_new[step_in_epoch-1]=acc
        loss_new[step_in_epoch-1]=loss
        end = time.time()
    ...
    #pd.DataFrame(acc_new).to_csv('/home/chemquiz/ai-science-training-series/04_modern_neural_networks/table_acc.csv',sep=' ')
    #pd.DataFrame(loss_new).to_csv('/home/chemquiz/ai-science-training-series/04_modern_neural_networks/table_loss.csv',sep=' ')
    #plt.plot(loss_new)
    #plt.xlabel('iteration')
    #plt.ylabel('accuracy')
    #plt.show()
    #plt.savefig('/home/chemquiz/ai-science-training-series/04_modern_neural_networks/fig.png')
...
def main():
    ...
    #########################################################################
    BATCH_SIZE = 400 # Changing the batchsize 
    N_EPOCHS = 1
    ...
    optimizer = tf.keras.optimizers.Adam(learning_rate=0.0001) #Changing the learning rate 
